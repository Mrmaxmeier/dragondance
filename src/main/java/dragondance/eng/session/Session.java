...
import dragondance.datasource.PCLogDataSource;
...
public class Session {
	...
	private Class<?> getDatasourceAdapter(int type) {
		switch (type) {
			...
			case CoverageDataSource.SOURCE_TYPE_PCLOG:
				return PCLogDataSource.class;
		}
	...
	public CoverageData addCoverageData(String fileName) throws FileNotFoundException {
		...
		try {
			dataSource = (CoverageDataSource) clazz.getDeclaredConstructor(String.class, String.class)
					.newInstance(fileName, this.imageName);
